...
const DeleteForm =(props) => {
  ...
  const fetchRsvps = async() => {
    ...
    const data = await axios.get(`api/rsvpfilter?familyName=${(newFamilyName)}`)
    .then(function(response){
  ...
  return( 
    ...
    <p>Select a family to view their party</p>  
    <select className='search-input' onChange={setOption}>
        <option key="select" style={{display: 'none'}} value="">Select An Item</option>
     {rsvps.map((item) => <option key={item} value={item}>{item}</option>)}
    </select>   
    </div>}
    {props.loading === false && rsvps.length === 0 && <div className='nav-sub-container'> <div className = 'alert alert-danger'> There are no current families in the system. Please create one for this feature to work correctly. </div> </div>}
    {newFamilyName.length > 0 &&<div className = 'create-form-container'>
    <ul key={newFamilyName} className ='initial-search-list'>
        {newFamilyList.map(item => (
          <div>
          <li key={item._id} className='initial-search-list-item' onClick={() => settingModal(item)} >
              {item.familyName} 
              <br></br>
              {item.rsvpCode}
              <br></br>
            ...
            {modal === true && item._id === id && <Modal value = {id} title="delete" deleteFunction={deleteID}></Modal>}
      </div>
        ))
      }
      </ul>   
    ...
    {errorMessage.length>1 && <div className='nav-sub-container'>
      <div className = 'alert alert-success'> This family record deletion was {errorMessage}</div>
    </div>}
